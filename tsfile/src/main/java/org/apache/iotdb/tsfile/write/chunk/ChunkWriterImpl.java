...
public class ChunkWriterImpl implements IChunkWriter {
  ...
  private List<Statistics> pageStatisticsList = new ArrayList<>();
  ...
  private void writePageToPageBuffer() {
    try {
      if (numOfPages == 0) { // record the firstPageStatistics
        ...
        this.firstPageStatistics.updateStatistics();
      ...
      numOfPages++;
      Statistics pageStats = pageWriter.getStatistics();
      pageStats.updateStatistics(); // 手动触发聚类元数据计算
      this.statistics.mergeStatistics(pageStats);
      this.pageStatisticsList.add(pageStats.clone());
